...
public class PostgresAffiliationRepositoryAdapter implements AffiliationRepository {
    ...
    @Override
    public Optional<Affiliation> findByEstablishmentIdAndProfessionalId(UUID establishmentId, UUID professionalId) {
        return jpaRepository.findByEstablishmentIdAndProfessionalId(establishmentId, professionalId).map(this::toDomain);
    }
        ...
        if (domain.getServiceOfferings() != null) {
            entity.setServiceOfferings(domain.getServiceOfferings().stream()
                    .map(so -> ServiceOfferingEntity.builder()
                            .affiliation(entity)
                            .providedService(ProvidedServiceEntity.builder().id(so.getProvidedServiceId()).build())
                            .price(so.getPrice())
