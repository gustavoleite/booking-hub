    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            if (path.contains("/v3/api-docs") || path.contains("/swagger-ui")
                    || path.contains("/api-docs") || path.contains("/webjars")) {
            ...
            if (path.matches("^/api/catalog/establishments/" + uuidPattern + "$")
                    && "GET".equals(request.getMethod().name())) {
                return chain.filter(exchange);
            }
            if (path.matches("^/api/catalog/professionals/" + uuidPattern + "$")
                    && "GET".equals(request.getMethod().name())) {
                return chain.filter(exchange);
            }
            if (path.matches("^/api/catalog/establishments/" + uuidPattern
                    + "/affiliations/professional/" + uuidPattern + "/schedule$")
                    && "GET".equals(request.getMethod().name())) {
                return chain.filter(exchange);
            }
            if ("/api/bookings/availability".equals(path)
                    && "GET".equals(request.getMethod().name())) {
                return chain.filter(exchange);
            }

            // Public review endpoints (listing and stats — no auth required)
            if (path.matches("^/api/reviews/professional/" + uuidPattern + "(/stats)?$")
                    && "GET".equals(request.getMethod().name())) {
                return chain.filter(exchange);
            }
            if (path.matches("^/api/reviews/establishment/" + uuidPattern + "(/stats)?$")
                    && "GET".equals(request.getMethod().name())) {
                return chain.filter(exchange);
            }
            ...
            try {
                ...
                return onError(exchange, "Gateway configuration error: " + e.getMessage(),
