    ...
    public SearchPage search(SearchFilter filter, int page, int size) {
        try {
            ...
            if (filter.getMinRating() != null) {
                boolQuery.filter(f -> f.range(r -> r.field("averageRating").gte(JsonData.of(filter.getMinRating()))));
            }
            if (filter.getMinPrice() != null) {
                boolQuery.filter(f -> f.range(r -> r.field("minPrice").gte(JsonData.of(filter.getMinPrice()))));
            }
            if (filter.getMaxPrice() != null) {
                boolQuery.filter(f -> f.range(r -> r.field("maxPrice").lte(JsonData.of(filter.getMaxPrice()))));
            }
            ...
            if (filter.hasGeo()) {
                boolQuery.filter(f -> f.geoDistance(g -> g
                        .field("geoPoint")
                        .location(loc -> loc.latlon(ll -> ll.lat(filter.getGeoLat()).lon(filter.getGeoLon())))
            ...
            SearchFilter.SortBy sortBy = filter.getSortBy() != null ? filter.getSortBy() : SearchFilter.SortBy.RELEVANCE;
            ...
            for (Hit<EstablishmentEsDocument> hit : response.hits().hits()) {
                EstablishmentDocument doc = hit.source() != null ? toDomain(hit.source()) : new EstablishmentDocument();

                Double distanceKm = null;
                if (filter.hasGeo() && finalSortBy == SearchFilter.SortBy.DISTANCE && !hit.sort().isEmpty()) {
                    try {
