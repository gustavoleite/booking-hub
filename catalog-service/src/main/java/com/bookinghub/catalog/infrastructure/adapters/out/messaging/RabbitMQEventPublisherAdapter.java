...
public class RabbitMQEventPublisherAdapter implements CatalogEventPublisher {
    ...
    public void publishEstablishmentCreated(Establishment establishment) {
        rabbitTemplate.convertAndSend(EXCHANGE, "establishment.created", toEstablishmentPayload(establishment));
    }

    @Override
    public void publishEstablishmentUpdated(Establishment establishment) {
        rabbitTemplate.convertAndSend(EXCHANGE, "establishment.updated", toEstablishmentPayload(establishment));
    }

    @Override
    public void publishAffiliationCreated(Affiliation affiliation, Professional professional, Establishment establishment) {
        rabbitTemplate.convertAndSend(EXCHANGE, "affiliation.created", toAffiliationPayload(affiliation, professional, establishment));
    }

    @Override
    public void publishAffiliationUpdated(Affiliation affiliation, Professional professional, Establishment establishment) {
        rabbitTemplate.convertAndSend(EXCHANGE, "affiliation.updated", toAffiliationPayload(affiliation, professional, establishment));
    }
    ...
    private AffiliationEventPayload toAffiliationPayload(Affiliation affiliation, Professional professional, Establishment establishment) {
                ...
                affiliation.getServiceOfferings() != null
                        ? affiliation.getServiceOfferings().stream()
                                .map(so -> new AffiliationEventPayload.ServiceOfferingPayload(
                                        ...
                                        serviceTitles.getOrDefault(so.getProvidedServiceId().toString(), ""),
                                        so.getPrice(),
        ...
        return new AffiliationEventPayload(
                ...
                affiliation.getEstablishmentId() != null ? affiliation.getEstablishmentId().toString() : null,
                affiliation.getProfessionalId() != null ? affiliation.getProfessionalId().toString() : null,
                professional.getName(),
                professional.getSpecialties() != null ? professional.getSpecialties() : Collections.emptyList(),
